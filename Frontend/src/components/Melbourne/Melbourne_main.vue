...
<script>
import Dropdown from "@/components/Melbourne/Dropdown.vue";
   ...
   data(){
    ...
    showData() {
      ...
      console.log(this.showChart)
       ...
       async getData() {
         try {
           const response = await axios.get('http://172.26.135.144:8080/melbourne');
       ...
       for (const age of this.ageGroups) {
         ...
         this.initChart2(this.pie_chart,this.RegisteredTotal,this.DefactoTotal,this.NotMarriedTotal,'Marital Status of Melbourne city in Pie Chart')
         this.initChart3(this.bar_chart, this.ageGroups,this.people_marriedArray,this.people_deFactoArray,this.people_notMarriedArray,'Marital Status of Melbourne city in Bar Chart')
         } catch (error) {
